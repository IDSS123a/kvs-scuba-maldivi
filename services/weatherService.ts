...
/**
 * Maps WMO weather codes from Open-Meteo to OpenWeatherMap-style descriptions and icons
 * for full compatibility with the existing UI components and icon assets.
 */
...
export async function fetchWeather(): Promise<WeatherData> {
  try {
    // Using Open-Meteo as it is a free, reliable, and key-less alternative
    // that avoids the common "fetch failed" issues associated with expired/invalid OWM keys.
    ...
    // Extract next 3 days for the forecast widget
    ...
    // Log the error but provide a graceful fallback to ensure the UI remains functional
    console.warn('Weather service failed, providing fallback data:', error);
    return {
