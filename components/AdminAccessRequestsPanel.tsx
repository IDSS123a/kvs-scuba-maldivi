...
export const AdminAccessRequestsPanel: React.FC<Props> = ({ lang = 'BS' }) => {
  ...
  const handleApprove = async (request: AccessRequest) => {
    ...
    try {
      ...
      const result = await approveUserWithPin(request.id, currentAdmin.uid);
      ...
      // Note: This alert is now concise and ideally should be replaced by a Modal or Toast in future
      const message = `
...
${t.pinCopied}
  ...
  const handleReject = async (request: AccessRequest) => {
    ...
    try {
      const result = await rejectUserAccessRequest(request.id, currentAdmin.uid, reason);
