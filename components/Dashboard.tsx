...
import { sendAttendanceConfirmationEmail } from '../services/emailService';
...
const Dashboard: React.FC<Props> = ({ lang, theme }) => {
  ...
  const handleConfirmAttendance = async () => {
    ...
    const user = userString ? JSON.parse(userString) : { displayName: 'Anonymous', email: 'N/A' };
    const logs = JSON.parse(localStorage.getItem('kvs_attendance_logs') || '[]');
    const attendanceRecord = { user: user.displayName, email: user.email, date: new Date().toISOString() };
    logs.push(attendanceRecord);
    localStorage.setItem('kvs_attendance_logs', JSON.stringify(logs));
    ...
    // Send email notification if email is valid
    if (user.email && user.email !== 'N/A') {
      try {
        console.log('📧 Sending attendance confirmation email to:', user.email);
        const emailSent = await sendAttendanceConfirmationEmail(user.email, user.displayName);
        if (emailSent) {
          console.log('✅ Attendance confirmation email sent successfully');
        } else {
          console.warn('⚠️ Email sending failed, but attendance is recorded');
        }
      } catch (error) {
    ...
    setTimeout(() => setAttendanceConfirmed(false), 5000);
  };
  ...
  const translations = {
    BS: {
      ...
      meetingTitle: "OBAVEZAN SASTANAK GRUPE",
      meetingHeader: "FINALNI DOGOVOR PRED POLAZAK",
      confirmAttendance: "Potvrdi Dolazak",
      confirmed: "DOLAZAK POTVRĐEN",
    ...
    EN: {
      ...
      meetingTitle: "MANDATORY GROUP MEETING",
      meetingHeader: "FINAL ARRANGEMENTS BEFORE DEPARTURE",
  ...
  return (
    <div className={`animate-in fade-in duration-700 ${theme === 'dark' ? 'bg-[#001219]' : 'bg-[#f8fdff]'}`}>
      ...
      >
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center text-white flex flex-col items-center justify-center fade-in">
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <button 
              ...
              className={`p-8 rounded-[40px] flex items-center gap-6 shadow-2xl transition-all hover:scale-[1.02] active:scale-95 ${theme === 'dark' ? 'bg-white/5 border border-white/10' : 'bg-white border border-cyan-50'}`}
            >
      ...
      <section className={`py-24 px-6 relative overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-[#001219]' : 'bg-white'}`}>
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            {/* Weather Card */}
            <div className={`glass-card rounded-[40px] p-10 space-y-8 border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-[#f0f9fa] border-cyan-50'}`}>
                  ...
                  <div className="flex-1 flex justify-around w-full">
                    {weather.forecast.map((day, i) => (
                      <div key={i} className="text-center">
                        <p className="text-[10px] font-black text-gray-400 mb-2">{day.date}</p>
            ...
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-4 gap-4">
                {[{ label: t.days, val: timeLeft.days }, { label: t.hours, val: timeLeft.hours }, { label: t.mins, val: timeLeft.mins }, { label: t.secs, val: timeLeft.secs }].map((item) => (
                  <div key={item.label} className={`rounded-[32px] p-6 text-center border ${theme === 'dark' ? 'bg-white/5 border-white/10' : 'bg-[#f8fdff] border-cyan-50'}`}>
                    <div className="text-5xl font-black tracking-tighter text-[#0a9396]">{item.val}</div>
                    <div className="text-[10px] uppercase font-black text-gray-400 mt-2">{item.label}</div>
                  </div>
                ))}
              </div>
            </div>
      ...
      <section className="py-24 px-6 bg-[#001219]">
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row gap-12 items-center">
            <div className="flex-1 space-y-6 text-white">
              <span className="text-[#ee9b00] font-black text-xs uppercase tracking-[0.4em]">{t.aboutMaldives}</span>
              ...
              {rates && (
                <div className="flex gap-4">
                   <div className="bg-emerald-500/10 p-6 rounded-3xl border border-emerald-500/20">
                      <p className="text-[10px] font-black text-emerald-500 uppercase mb-1">EUR / BAM</p>
                      <p className="text-xl font-black">{rates.BAM.toFixed(2)} KM</p>
                   </div>
                   <div className="bg-cyan-500/10 p-6 rounded-3xl border border-cyan-500/20">
                      <p className="text-[10px] font-black text-cyan-500 uppercase mb-1">EUR / USD</p>
                      <p className="text-xl font-black">${rates.USD.toFixed(2)}</p>
                   </div>
                </div>
              )}
            </div>
            <div className="flex-1 w-full grid grid-cols-2 gap-4">
                {countryData && [
                  ...
                  <div key={i} className="bg-white/5 border border-white/10 p-6 rounded-[32px] space-y-2">
                    ...
                    <p className="text-xs font-black text-gray-400 uppercase">{item.label}</p>
      ...
      <section className="py-24 px-6">
        <div className="max-w-5xl mx-auto glass-card rounded-[60px] p-16 bg-[#005f73] text-white text-center space-y-10 shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 w-80 h-80 bg-white/5 rounded-full -translate-y-1/2 translate-x-1/2" />
          <CalendarDays className="w-12 h-12 text-[#ee9b00] mx-auto" />
          <h2 className="text-4xl md:text-6xl font-black leading-tight">{t.meetingHeader}</h2>
          <div className="space-y-4">
            <p className="text-4xl font-black text-[#ee9b00]">24.12.2025. @ 19:00</p>
            <p className="text-2xl font-bold">Lokacija: Vistafon</p>
          </div>
          <button 
            onClick={handleConfirmAttendance}
            className={`px-12 py-5 rounded-full font-black text-xs uppercase tracking-[0.3em] transition-all shadow-xl ${
              attendanceConfirmed ? 'bg-emerald-500' : 'bg-white text-[#005f73] hover:bg-[#ee9b00] hover:text-white'
            }`}
          >
            {attendanceConfirmed ? t.confirmed : t.confirmAttendance}
          </button>
        </div>
      </section>
          ...
          <div className="w-full max-w-2xl rounded-[60px] bg-[#001219] text-white p-12 md:p-16 relative border border-white/10 shadow-2xl">
            ...
            <div className="flex flex-col gap-8">
              <div className="flex items-center gap-6">
                ...
                <div>
                  <span className="text-[#ee9b00] font-black text-xs uppercase tracking-widest">{modal.subtitle}</span>
                  <h3 className="text-4xl font-black mt-2">{modal.title}</h3>
                </div>
              </div>
              <p className="text-lg leading-relaxed text-gray-300">{modal.description}</p>
              <div className="flex flex-wrap gap-3">
                {modal.tags.map(tag => (
                  <span key={tag} className="px-4 py-2 bg-cyan-500/10 text-cyan-400 font-bold text-[10px] uppercase rounded-full border border-cyan-500/20">{tag}</span>
                ))}
              </div>
            </div>
          </div>
